  ...
  const caseStudies = [
    {
      ...
      results: [
        '50% faster TTI on landing pages',
    ...
    },
    {
      ...
      results: [
        '70% faster store load times',
        '20% increase in merchant sales',
                      ...
                      {Object.entries(selectedOptimization.metrics).map(([metric, value]: any[]) => (
                        <div key={metric} className="flex items-center justify-between">
                          ...
                          <span className={`text-sm font-semibold ${
                            ...
                            {value > 0 ? '+' : ''}{value}{metric.includes('score') ? '' : metric.includes('Size') ? 'MB' : metric.includes('Time') ? 'ms' : 's'}
                          </span>
                        </div>
                      ))}
                    </div>
            ...
            {caseStudies.map((study: any, index: number) => (
              <div key={index} className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className={`px-3 py-1 rounded-full text-sm ${colorClasses[study.color]}`}>
                    {study.architecture}
                  </div>
                </div>
